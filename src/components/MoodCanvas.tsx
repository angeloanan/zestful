...
export interface MoodCanvasProp {
  mood?: Mood | undefined
}
export const MoodCanvas: ParentComponent<MoodCanvasProp> = ({ mood, ref, children }) => {
  return (
    <div
      class='relative grid aspect-square w-full grid-cols-10 grid-rows-10 gap-1 rounded-3xl bg-neutral-200 p-3 text-center font-sansTight text-xs'
      ref={ref}
    >
      <Show when={mood}>
        {(mood) => {
          const left = lerp(2, 96.5, (mood().pleasantness + 1) / 2)
          const bottom = lerp(2, 96.5, (mood().energy + 1) / 2)
